...
// 复位
hi_void wifi_setup_reset(hi_void)
{
    hi_watchdog_disable();
    hi_udelay(3000);
...
// 延时  命令之间加三秒延时保证执行完成
hi_void wifi_delay(hi_void)
{
    hi_udelay(3000000);
}

//设置mac地址
hi_u32 wifi_set_macaddr(hi_void)
{
    hi_uchar mac_addr[6]; /* 6 mac len */
    for (hi_u32 i = 0; i < 6; i++) {
        mac_addr[i] = 1;
    }
...
// 关联ap
hi_u32 wifi_sta_connect(hi_void)
{
    hi_wifi_assoc_request assoc_req = {0};
    hi_char * ssid = "8189FAST_TEST";
    hi_char * bssid = "000102030405";

    memcpy_s(&assoc_req.ssid, HI_WIFI_MAX_SSID_LEN + 1, ssid, strlen(ssid) + 1);
    memcpy_s(&assoc_req.bssid, HI_WIFI_MAC_LEN, bssid, strlen(bssid) + 1);
...
/*
 * 目的：模仿联网流程编写demo，调用lwip、dhcp接口复现客户调试卡死现象
 * 调用接口顺序：
 * AT+RST
 * AT+MAC=8c:22:33:52:55:25    wifi_set_macaddr
 * AT+STARTSTA
 * AT+SCAN
 * AT+SCANRESULT
 * AT+CONN="8189FAST_TEST",,0 
 * AT+DHCP=wlan0,1
 * AT+IFCFG
 * AT+SYSINFO
 * AT+PING=192.168.1.1
 * 
 */
static hi_void *wifi_demo_task_body(hi_void *param)
{
    /* Releases the app_main stack memory. */
    hi_unref_param(param);
//    wifi_setup_reset();
...
hi_void app_main(hi_void)
{
    ...
    attr.task_prio = APP_DEMO_RELEASE_MEM_TASK_PRIO;
    attr.task_name = (hi_char*)"wifi_demo_task";
    ret = hi_task_create(&task_id, &attr, wifi_demo_task_body, HI_NULL);
    if (ret != HI_ERR_SUCCESS) {
        printf("Falied to create app_demo_second task:0x%x\n", ret);
    }
